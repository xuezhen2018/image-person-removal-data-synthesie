    ...
    def validate(self, epoch,val_len_data):
        ...
        with torch.no_grad():
            for i in range(val_len_data):
                ...
                val_inputs, val_mask, val_GT = next(self.val_loader)  # 这里还是tensor格式
    ...
    def test(self,val_len_data):
        ...
        with torch.no_grad():
            for i in range(val_len_data):
                ...
                # outputs = self.model(self.norm(val_inputs), self.norm(val_mask)) #改动
                ...
                self.save_output(
                    ...
                    num = i,
                    verbose=False)
        print("PSNR:%.4f,SSIM:%.4f,LPIPS:%.4f,RMSE:%.4f,RMSER:%.4f" % (psnr_meter.avg, ssim_meter.avg,sum(LPIPS_meter)/val_len_data,rmse_meter.avg,rmsew_meter.avg))

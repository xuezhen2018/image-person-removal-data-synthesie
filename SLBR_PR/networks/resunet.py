...
class CoarseEncoder(nn.Module):
    ...
    def forward(self, x, x_m):
        ...
        for i,nets in enumerate(zip(self.down_convs,self.down_convs_m)):
            ...
            x_m , before_pool_m = d_conv_m(x_m)
            encoder_outs_im.append(before_pool_im)
            encoder_outs_m.append(before_pool_m)

        return x,x_m, encoder_outs_im,encoder_outs_m
...
class CoarseDecoder(nn.Module):
    ...
    def forward(self, bg,encoder_outs_im=None,encouder_out_m=None):
        bg_x = bg
        bg_outs = []
        for i, up_convs in enumerate(self.up_convs_bg):
            ...
            if self.use_att:
                ...
                bg_before_pool_m = self.atts_bg[i](before_pool_m)

            bg_x = up_bg(bg_x, bg_before_pool_im, bg_before_pool_m.detach())
            bg_outs.append(bg_x)
...
class SLBR(nn.Module):
    ...
    def __init__(self, args, in_channels=3, depth=5, shared_depth=2, blocks=1,
        ...
        # refinement stage
        if args.use_refine:
            self.refinement = Refinement(in_channels=4, out_channels=3, shared_depth=1, n_cff=args.k_refine,
    ...
    def forward(self, in_image,in_mask):
        ...
        image_code, mask_code,before_pool_im,before_pool_m = self.encoder(in_image,in_mask)
        unshared_before_pool_im = before_pool_im  # [: - self.shared]
        unshared_before_pool_m = before_pool_m  # [: - self.shared]

        # decoder
        im = self.shared_decoder(image_code)

        ims = self.coarse_decoder(im,unshared_before_pool_im,unshared_before_pool_m)
        im = ims[0]
        ...
        if self.refinement is not None:
            ...
            refine_bg = self.refinement(in_image, coarser, reconstructed_mask, None, dec_feats)
            refine_bg = (torch.tanh(refine_bg) + in_image).clamp(0, 1)  # coarser

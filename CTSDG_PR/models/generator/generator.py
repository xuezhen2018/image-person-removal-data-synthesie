...
class Generator(nn.Module):
    ...
    def forward(self, input_image, input_edge, mask):
        ...
        for _ in range(7, 0, -1):
            ...
            dc_tecture_mask = F.interpolate(dc_tecture_mask, scale_factor=2, mode='bilinear')
            threshold = 0.003
            ones = dc_tecture_mask >= threshold
            zeros = dc_tecture_mask < threshold

            dc_tecture_mask.masked_fill_(ones, 1.0)
            dc_tecture_mask.masked_fill_(zeros, 0.0)
        ...
        for _ in range(7, 0, -1):
            ...
            dc_structure_masks = F.interpolate(dc_structure_masks, scale_factor=2, mode='bilinear')
            threshold = 0.003
            ones = dc_tecture_mask >= threshold
            zeros = dc_tecture_mask < threshold

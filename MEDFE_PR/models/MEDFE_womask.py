...
class MEDFE(BaseModel):
    def __init__(self, opt):
        ...
        if self.isTrain:
            # if opt.continue_train :
                ...
                self.load_networks()
                # self.load_networks(self.netDE)
                # self.load_networks(self.netMEDFE)
                # self.load_networks(self.netD)
                # self.load_networks(self.netF)
    ...
    # Assuming mask has shape [batch_size, channels, height, width]
    
        mask = mask[:, 0, :, :]  # Select the first channel for each sample in the batch
        mask = torch.unsqueeze(mask, 1)  # Add a new channel dimension
        mask = mask.byte()  # Convert the tensor to byte type
    
        return mask


    def set_input(self, GT, input_St,input_De, mask):

        self.Gt_DE = GT.to(self.device)
        self.Gt_ST = input_St.to(self.device)
        self.input_DE = input_De.to(self.device)

        self.mask_global = self.mask_process(mask.to(self.device)) #改
        # self.mask_global = mask.to(self.device).byte()

        self.Gt_Local = GT.to(self.device)
        ...
        self.inv_ex_mask = torch.add(torch.neg(self.ex_mask.float()), 1).float()#改
        # print(self.inv_ex_mask.shape)

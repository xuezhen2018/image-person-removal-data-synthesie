...
class BASE(nn.Module):
    def __init__(self, inner_nc):
        ...
        model = [se]
        gus = util.gussin(2.0).cuda()
        # print("gus",gus.shape)
        self.gus = torch.unsqueeze(gus, 1).double()#原

        # print("self.gus",self.gus.shape)
    ...
    def forward(self, x):
        ...
        Nonparm = Selfpatch()
        out_32 = self.model(x)
        # print("out_32_1",out_32.shape)
        b, c, h, w = out_32.size()
        # print('B',b,'c',c,'h',h,'w',w)
        gus = self.gus.float()
        # print('gus_1',gus.shape)
        gus_out = out_32[0].expand(h * w, c, h, w)
        # gus_out = out_32[0].expand(h * w *4, c, h, w)
        # print('gus_out_1',gus_out.shape)
        gus_out = gus * gus_out
        # print('gus_out_2',gus_out.shape)
        gus_out = torch.sum(gus_out, -1)
        # print('gus_out_3',gus_out.shape)
        gus_out = torch.sum(gus_out, -1)
        # print('gus_out_4',gus_out.shape)
        gus_out = gus_out.contiguous().view(b, c, h, w)
        # print('gus_out_5',gus_out.shape)
...
class PCconv(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        # print("x_cat_fuse",x_cat_fuse.shape)
